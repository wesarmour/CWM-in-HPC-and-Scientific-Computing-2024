...
void runTest(int argc, char **argv)
{
    ...
    for (unsigned int i = 0; i < SIGNAL_SIZE; ++i)
    {
        ...
        if(i > SIGNAL_SIZE/4 && i < (SIGNAL_SIZE)/4+FILTER_KERNEL_SIZE) {
		h_signal[i].x = 1.0f;
	} else {
		h_signal[i].x = 0.0f;
	}
    ...
    for (unsigned int i = 0; i < FILTER_KERNEL_SIZE; ++i)
    {
        ...
        h_filter_kernel[i].x = ((float)FILTER_KERNEL_SIZE)/((float)(FILTER_KERNEL_SIZE-i));
    ...
    for(int i = 0; i < SIGNAL_SIZE; i++) printf("%d %f %f\n", i, h_convolved_signal_ref[i].x, h_convolved_signal[i].x);
